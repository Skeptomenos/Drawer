...
| Status | Test ID | Test Case | Expected Result | Notes |
|--------|---------|-----------|-----------------|-------|
| [ ] | SETUP-004 | Create MockSettingsManager | Mock compiles and can be instantiated | |
...
| Status | Test ID | Test Case | Expected Result | Notes |
|--------|---------|-----------|-----------------|-------|
| [ ] | SETUP-008 | Verify test target runs | Simple assertion passes (1+1=2) | Smoke test |
...
| Status | Test ID | Test Case | Expected Result | Notes |
|--------|---------|-----------|-----------------|-------|
| [ ] | MBM-001 | Initial state isCollapsed is true | isCollapsed == true on init | CRITICAL |
...
| [ ] | MBM-013 | Auto-collapse timer restarts on settings change | Timer restarts when delay changes | MEDIUM |
| [ ] | MBM-014 | Expand image is correct for LTR | chevron.left for LTR expand | MEDIUM |
| [ ] | MBM-015 | Collapse image is correct for LTR | chevron.right for LTR collapse | MEDIUM |
...
| [ ] | SET-005 | Default alwaysHiddenEnabled is false | alwaysHiddenEnabled == false | CRITICAL |
| [ ] | SET-006 | Default useFullStatusBarOnExpand is false | useFullStatusBarOnExpand == false | CRITICAL |
| [ ] | SET-007 | Default showOnHover is false | showOnHover == false | CRITICAL |
...
| [ ] | DRM-007 | updateItems clears lastError | lastError set to nil on update | HIGH |
| [ ] | DRM-008 | clearItems removes all | clearItems() empties items array | CRITICAL |
| [ ] | DRM-009 | clearItems clears lastError | clearItems() sets lastError to nil | HIGH |
| [ ] | DRM-010 | setLoading(true) sets isLoading true | isLoading == true | HIGH |
...
| [ ] | DRM-017 | toggle() from visible hides | toggle() when visible hides | CRITICAL |
...
| [ ] | HVM-011 | isInMenuBarTriggerZone below menu bar | Point below menu bar is not in zone | MEDIUM |
| [ ] | HVM-012 | isInDrawerArea inside frame | Point inside frame returns true | MEDIUM |
| [ ] | HVM-013 | isInDrawerArea outside frame | Point outside frame returns false | MEDIUM |
| [ ] | HVM-014 | isInDrawerArea with expanded hit area | 10px expansion works | MEDIUM |
...
| Status | Test ID | Test Case | Expected Result | Notes |
|--------|---------|-----------|-----------------|-------|
| [ ] | DRI-001 | Init from CapturedIcon | DrawerItem correctly initializes from CapturedIcon | LOW |
...
| Status | Test ID | Test Case | Expected Result | Notes |
|--------|---------|-----------|-----------------|-------|
| [ ] | PRT-001 | Accessibility displayName | displayName is "Accessibility" | LOW |
| [ ] | PRT-002 | ScreenRecording displayName | displayName is "Screen Recording" | LOW |
| [ ] | PRT-003 | Accessibility description | Description text correct | LOW |
| [ ] | PRT-004 | ScreenRecording description | Description text correct | LOW |
